...
router.get('/ingreso_admins', (req, res) => {
    conexion.query(`
        ...
        JOIN administradores ON ingresos_admins.id_ad = administradores.id_ad
    `, (error, results) => {
        if (error) {
            console.error('Error al obtener los ingresos:', error);
            res.status(500).send('Error al obtener los ingresos');
...
const bcryptjs = require('bcryptjs'); // Asegúrate de que bcryptjs esté instalado

router.post('/actualizar_admin/:id', async (req, res) => {
    try {
        const id = req.params.id;
        ...
        if (contrasena) {
            passwordHash = await bcryptjs.hash(contrasena, 8);
        }
        ...
        // Solo incluir la contraseña encriptada si se proporciona
        if (passwordHash) {
            updatedAdmin.contrasenia_ad = passwordHash;
        }
