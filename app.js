...
app.post('/register', async (req, res) => {
    ...
    const  institucion=req.body.registerInstitucion;
    ...
    connection.query('SELECT correo_us FROM usuarios WHERE correo_us = ?', [correo], (error, results) => {
        ...
        connection.query('INSERT INTO usuarios SET ?', {
            ...
            ciudad_us: ciudad,
            institucion_us: institucion,
            empleo_us: empleo,
            correo_us: correo,
            contrasenia_us: passwordHash
        }, (error, results) => {
            if (error) {
                console.log(error);
...
app.post('/auth', async (req, res) => {
    ...
    if (user && pass) {
        connection.query('SELECT * FROM usuarios WHERE correo_us = ?', [user], async (error, results) => {
            ...
            if (results.length == 0 || !(await bcryptjs.compare(pass, results[0].contrasenia_us))) {
                return res.render('login', {
                    alert: true,
                    alertTitle: "Error",
...
app.post('/registro_usuarios', async (req, res) => {
    ...
    const institucion=req.body.institucion_us;
...
app.post('/authadmin', async (req, res) => {
    ...
    const passad = req.body.pass;  // Esto ahora coincide con el input del formulario
    if (userad && passad) {
        connection.query('SELECT * FROM administradores WHERE correo_ad = ?', [userad], async (error, results) => {
            ...
            // Verifica si el usuario existe y si la contraseña es correcta
            if (results.length == 0 || !(await bcryptjs.compare(passad, results[0].contrasenia_ad))) {
                ...
                connection.query(
                    'INSERT INTO ingresos_admins (hora_ingad,fecha_ingad,id_ad) VALUES (?, ?, ?)', 
                    [time, date, userId], 
                    (error, result) => {
                        if (error) {
                            return res.render('login_admin', {
